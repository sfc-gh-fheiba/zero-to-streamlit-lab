...
if st.session_state.chosen_restaurant != '':
    ...
    if feedback_submitted:
        with st.spinner("Submitting"):
            session.sql("CREATE DATABASE IF NOT EXISTS MARKETPLACE_RESTAURANTS_FEEDBACK;").collect()
            session.sql("CREATE TABLE IF NOT EXISTS MARKETPLACE_RESTAURANTS_FEEDBACK.PUBLIC.USER_FEEDBACK (LOCATION_ID VARCHAR, LOCATION_NAME VARCHAR, LOCATION_ADDRESS VARCHAR, USER_COMMENTS VARCHAR, USER_RATING VARCHAR)").collect()    
            ...
            final_restaurant_feedback.write.save_as_table("USER_FEEDBACK", mode="append")
            
            st.success("✅ Successfully saved your rating!")
            # Celebrate: https://docs.streamlit.io/library/api-reference/status/st.snow
            st.snow()

            feedback_history = session.table("USER_FEEDBACK").collect()
